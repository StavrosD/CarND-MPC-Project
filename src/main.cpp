    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
	  ...
	  if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          /*
          * COMPLETED: Calculate steering angle and throttle using MPC.
          *
          * Both are in between [-1, 1].
          *
          */
          double steer_value_simulator = j[1]["steering_angle"];
		  
		  // The simulator angles are counterwise while the MPC angles are anti-counterwise.
		  double steer_value_mpc = -steer_value_simulator;
          double throttle_value = j[1]["throttle"];


          Eigen::VectorXd local_x(ptsx.size());
          Eigen::VectorXd local_y(ptsy.size());
          ...
          // std::cout << "v " << px << " v_predicted " << x_predicted << " t " << throttle_value <<std::endl;
		
		// move from global (unity) to local coordinates
		// unity measures angles counterwise, the model needs anti-counterwise angles
		  ...
		  // on local coordinates the vehicle is always at x=0, y=0 and psi=0
          // the conversion from global to local coordinates simplifies the calculations
		  state << 0, 0, 0, v_predicted, cte, epsi;
		...
		// I use the minus sign to make the conversion
		  ...
		  // std::cout << "42[\"steer\"," + msgJson.dump(4) + "]" << std::endl;
